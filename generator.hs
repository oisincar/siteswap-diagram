...
--   num balls -> throw height -> [state strings]
getAllNodes :: Int -> Int -> [String]
...
graph b allNodes = zip allNodes $ map findNodes allNodes
  where
    findNodes :: String -> [(Int, String)]
    findNodes (x:xs)
     | x == '0' = [(0, xs ++ "0")]
...
--    throwHeight -> [states] -> [lines]
showStates :: Int -> [String] -> [String]
showStates h st = (map showLine (reverse [0..h-1]))
  where showLine y = '|' : concatMap (\s -> (s !! y) : "|") st


showGraph revGr = [showLine (revGr !! x) x (length revGr) | x <- [0..(length revGr) -1]]
  where
